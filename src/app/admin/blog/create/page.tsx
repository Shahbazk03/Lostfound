...
import { ArrowLeft, Save, Layout, Eye, Settings, Image as ImageIcon, Send } from "lucide-react";
...
export default function CreatePostPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/blog/categories").then(res => res.json()).then(data => setCategories(data.categories || []));
    fetch("/api/admin/blog/tags").then(res => res.json()).then(data => setTags(data.tags || []));
  }, []);
  ...
  const handleSave = async (status: string) => {
    ...
    try {
      const res = await fetch("/api/admin/blog/posts", {
        ...
        body: JSON.stringify({ ...formData, status, categoryId: formData.categoryId ? parseInt(formData.categoryId) : null }),
      });
      ...
      <div className="h-16 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between px-6 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">Draft</span>
          </div>
        </div>
