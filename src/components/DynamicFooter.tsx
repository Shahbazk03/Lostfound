...
export default function DynamicFooter() {
  ...
  const desc = settings.metadata?.footerDescription || "Empowering communities globally to connect lost belongings with their rightful owners. Trusted, secure, and fast.";
  const copyright = settings.metadata?.copyrightText || `© ${new Date().getFullYear()} ${orgName} Inc. All rights reserved.`;
  const fb = settings.metadata?.facebookUrl || "#";
  const tw = settings.metadata?.twitterUrl || "#";
  const contactEmail = settings.contactEmail || "hello@lostfound.com";
  const contactPhone = settings.supportPhone || "+1 (555) 123-4567";
  ...
  const footerGroups: FooterGroup[] = Array.isArray(settings.metadata?.footerGroups) && settings.metadata.footerGroups.length > 0
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-10 mb-16 pb-12 border-b border-slate-800/60">
          ...
          <div className="flex flex-col items-start lg:items-end text-left lg:text-right">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-3xl font-extrabold text-white tracking-tight">{orgName}</span>
            </div>
