...
export default function HomepageVisuals({ data }: { data: HomepageData }) {
  const {
    hero,
    statisticsList,
    categoriesList,
  ...
  const renderHeadline = () => {
    ...
    const words = hero.highlightedWords.split(",").map((w: string) => w.trim());
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B1120] text-slate-900 dark:text-slate-200 selection:bg-emerald-500/30 selection:text-emerald-900 dark:selection:text-emerald-200 font-sans w-full">
            ...
            <ScrollReveal animation="slide-in" className="max-w-2xl z-10 relative">
              ...
              {hero.trustBadges && hero.trustBadges.length > 0 && (
                <div className="flex flex-wrap items-center gap-6 text-sm font-semibold text-slate-500 dark:text-slate-400">
                  {hero.trustBadges.map((badge: string, idx: number) => (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`grid grid-cols-2 lg:grid-cols-${Math.min(Math.max(statisticsList?.length || 4, 1), 4)} gap-8 divide-x divide-slate-200 dark:divide-slate-800`}>
            {statisticsList?.length > 0 ? (
              statisticsList.filter(s => s.isActive !== false).map((stat, i) => (
                <ScrollReveal key={stat.id || i} delay={i * 0.1} className="text-center px-4">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {categoriesList?.length > 0 ? (
              categoriesList.filter(c => c.isActive !== false).map((cat, i) => {
                // @ts-ignore
                const IconComponent = Icons[cat.icon] || Package;
                return (
                  <ScrollReveal key={cat.id || i} delay={i * 0.05}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {featuresList?.length > 0 ? (
              featuresList.filter(f => f.isActive !== false).map((feature, i) => {
                // @ts-ignore
                const IconComponent = Icons[feature.icon] || Shield;
                return (
                  <ScrollReveal key={feature.id || i} delay={i * 0.1} className="bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl p-8 rounded-3xl border border-slate-200/60 dark:border-slate-700/50 hover:bg-white dark:hover:bg-slate-800 transition-colors h-full">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {testimonialsList?.length > 0 ? (
              testimonialsList.filter(t => t.isActive !== false).map((testimonial, i) => (
                <ScrollReveal key={testimonial.id || i} delay={i * 0.15} className="bg-slate-50 dark:bg-slate-900 p-10 rounded-[2rem] border border-slate-200 dark:border-slate-800 flex flex-col h-full">
      ...
      {(!globalNetwork || globalNetwork.isActive !== false) && (
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
            <ScrollReveal>
              ...
              <h2 className="text-4xl md:text-5xl font-extrabold text-white mb-6">
                {globalNetwork?.title || "Global Recovery Network"}
              </h2>
              <p className="text-xl text-slate-400 max-w-2xl mx-auto mb-12">
                {globalNetwork?.description || "Our platform operates across borders. See how items are being recovered worldwide in real-time."}
              </p>
            </ScrollReveal>
            ...
            <ScrollReveal delay={0.2} className="relative aspect-video max-w-4xl mx-auto bg-slate-800/50 rounded-3xl border border-slate-700 overflow-hidden shadow-2xl flex items-center justify-center">
               ...
               {globalNetwork?.statistics?.length > 0 ? (
                 <div className="absolute inset-0 flex items-center justify-around z-10 px-12">
                    {globalNetwork.statistics.map((stat: any, i: number) => (
                      <div key={i} className="bg-slate-900/80 px-6 py-4 rounded-2xl backdrop-blur-md border border-slate-700 text-center">
                        <div className="text-3xl font-extrabold text-white mb-1">{stat.value}</div>
                        <div className="text-sm font-medium text-emerald-400 uppercase tracking-wide">{stat.label}</div>
                      </div>
                    ))}
                 </div>
               ) : (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {pricingPlansList?.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mt-12">
              {pricingPlansList.filter(p => p.isActive !== false).map((plan, i) => (
                <ScrollReveal key={plan.id || i} delay={i * 0.2} className={`relative bg-slate-900 dark:bg-slate-900 rounded-[2.5rem] p-10 border transition-all ${plan.isPopular ? 'border-emerald-500 shadow-2xl shadow-emerald-500/20 scale-105 z-10' : 'border-slate-800 shadow-xl'} overflow-hidden h-full flex flex-col`}>
