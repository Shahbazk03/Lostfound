...
export function GoogleLoginButton({ action, variant = "default" }: { action: "login" | "register", variant?: "default" | "icon" | "navbar" }) {
  const router = useRouter();

  const handleGoogleLogin = () => {
    // Redirect to the API endpoint which will redirect to Google's OAuth
  ...
  return (
    ...
    >
      <svg className={variant === "default" ? "w-5 h-5" : "w-4 h-4"} viewBox="0 0 24 24">
        <path
      ...
      {variant === "default" && (action === "login" ? "Sign in with Google" : "Sign up with Google")}
      {variant === "navbar" && "Continue with Google"}
    </button>
  );
}
